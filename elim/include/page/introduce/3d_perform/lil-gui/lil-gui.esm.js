...
function normalizeColorString(string) {
  ...
  if ((match = string.match(/(#|0x)?([a-f0-9]{6})/i))) {
    ...
    (match = string.match(/rgb\(\s*(\d*)\s*,\s*(\d*)\s*,\s*(\d*)\s*\)/))
  ) {
    result =
...
const ARRAY = {
  ...
  toHexString([r, g, b], rgbScale = 1) {
    ...
    const int =
      ((r * rgbScale) << 16) ^ ((g * rgbScale) << 8) ^ ((b * rgbScale) << 0);
...
class NumberController extends Controller {
  ...
  updateDisplay() {
    ...
    if (!this._inputFocused) {
      this.$input.value =
        this._decimals === undefined ? value : value.toFixed(this._decimals);
    }
  ...
  save(recursive = true) {
    ...
    if (recursive) {
      this.folders.forEach((f) => {
        if (f._title in obj.folders) {
          throw new Error(
            `Cannot save GUI with duplicate folder "${f._title}"`
          );
        }
